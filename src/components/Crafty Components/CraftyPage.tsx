...
const CraftyPage = () => {
  ...
  return (
    <div className="flex items-center gap-x-20 px-24 pb-10 pt-40">
      <div className="font-raleway flex h-full w-full flex-col items-start justify-start gap-y-24 leading-8">
        <div className="flex w-full flex-col items-start justify-start gap-y-6">
          <h1 className="mb-3 text-title font-bold">Craft Content</h1>
          <div className="flex w-full items-center justify-start gap-x-12">
            <input
              value={input}
              onChange={(e) => setInput(e.target.value)}
              className="h-20 w-full rounded-xl bg-input-gray px-6 text-xl font-bold text-black placeholder:text-black placeholder:text-opacity-50"
            ...
            <button
              ...
              className={`h-12 w-36 transform rounded-full text-2xl transition-transform hover:scale-105 active:scale-95 ${canCraft ? 'bg-button-blue font-bold text-white' : 'border-[1.5px] border-black bg-none font-normal text-main-black'}`}
            >
