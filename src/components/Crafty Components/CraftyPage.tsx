...
const CraftyPage = () => {
  ...
  const [buttonStyle, setButtonStyle] = useState<string[]>([
    "bg-none border-black border-[1.5px] font-normal text-main-black",
    "black",
  ]);

  useEffect(() => {
    if (input.trim() !== "") {
      setButtonStyle(["bg-button-blue font-bold text-white", "white"]);
    } else {
      setButtonStyle([
        "bg-none border-black border-[1.5px] font-normal text-main-black",
        "black",
        ...
        <div className="flex flex-col justify-start items-start gap-y-6 w-full">
          ...
          <div className="flex w-full gap-x-12 justify-start items-center">
            <input
              ...
              className="w-4/5 rounded-xl h-20 bg-input-gray px-6 placeholder:text-black placeholder:text-opacity-50 text-black font-bold text-xl"
            ...
            <button
              ...
              onClick={() => {}}
              className={`h-12 w-36 rounded-full text-2xl transition-transform transform hover:scale-105 active:scale-95 ${buttonStyle[0]}`}
            >
